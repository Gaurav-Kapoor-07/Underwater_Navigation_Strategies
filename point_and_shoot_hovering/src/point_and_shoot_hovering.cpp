#include <point_and_shoot_hovering/point_and_shoot_hovering.h>
// #include <cmath>
...
void PointAndShootHovering::current_pos_cb(monsun_msgs::Xsens700Position::ConstPtr const& msg)
{
    if(enable_) {
        ...
        {  
            ...
            ROS_INFO("point_and_shoot_hovering: dist to target: %f m", dist * 1000.0);
        ...
        if(counter < total_count && check_heading()) {
            ...
            ROS_INFO_THROTTLE(15, "point_and_shoot_hovering: counter and heading okay!");
            ...
            if(dist > dist_to_waypoint_threshold_m_ / 1000.0) {
                ...
                if (abs(current_heading_ - angle) > heading_threshold_degs_ && l == 0)  {
                    ...
                    ROS_INFO("point_and_shoot_hovering: heading error: %f degrees", current_heading_ - angle);
                    ...
                    speed_pub.publish(speed_msg);
                        ...
                        dist = sqrt(pow(71.5 * (target_lon_ - current_lon_), 2) +
                            pow(111.3 * (target_lat_ - current_lat_), 2));
                        ROS_INFO_THROTTLE(15, "point_and_shoot_hovering: dist to target: %f m", dist * 1000.0);
                        ///
                        ...
                        if (p == 1)
                        {
                            ...
                            ROS_INFO("point_and_shoot_hovering: distance to waypoint = %f m!", dist_to_waypoint_m); 

                            time_to_waypoint_s = dist_to_waypoint_m / assumed_speed_m_p_s_;

                            ROS_INFO("point_and_shoot_hovering: time to waypoint = %f s!", time_to_waypoint_s);

                            total_count = round(time_to_waypoint_s / timeout_secs_);
                            ROS_INFO("point_and_shoot_hovering: total count: %d", total_count);
                        ...
                        ROS_INFO("point_and_shoot_hovering: initializing timeout");
                        return;
                        }
                        ...
                        if (dt_timeout_s >= timeout_secs_) {
                            ...
                            l = 0;
                            counter += 1;
                            ROS_INFO("point_and_shoot_hovering: timeout, correcting heading!");
                            ROS_INFO("point_and_shoot_hovering: count left: %d", total_count - counter);
                        }
                }
            }
            else
            ...
            heading_pub.publish(heading_msg);
            speed_pub.publish(speed_msg);
            ROS_INFO("point_and_shoot_hovering: stopping and disabling heading_ctrl as all waypoints reached!");
