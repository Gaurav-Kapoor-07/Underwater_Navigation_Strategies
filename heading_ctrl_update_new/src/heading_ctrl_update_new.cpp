#include <heading_ctrl_update_new/heading_ctrl_update_new.h>
    ...
    : nhs(handle), nhp(private_handle)
{
    ...
    // // Signal heading reached
    // signal_heading_reached = nhp.advertise<std_msgs::Bool>("signal_heading_reached", 1);
    ...
    // nhp.param<f64>("threshold_degs_", threshold_degs_, 1.0);

    nhp.param<f64>("heading_correction", heading_correction, 5.0);
}
...
void HeadingCtrl::ctrl_cb(geometry_msgs::Vector3ConstPtr const& meas)
{
    ...
    f64 e = ref.heading - (meas->z + heading_correction);
    
    // // Signal heading reached
    // std_msgs::Bool signal_heading;
    
    // if (i)
    // {
    //     ROS_INFO("heading_ctrl_update_new: turning towards the required heading");
    //     ROS_INFO("heading_ctrl_update_new: heading error = %f degrees", e);

    //     // Signal heading reached
    //     signal_heading.data = true;
    //     signal_heading_reached.publish(signal_heading);
    ...
    // if (abs(e) <= threshold_degs_)
    // {
    //     if (j)
    //     {
    //         ROS_WARN("heading_ctrl_update_new: required heading already within limits!");
    //         ROS_WARN("heading_ctrl_update_new: heading error = %f degrees", e);
    ...
    //     //     ROS_WARN("heading_ctrl_update_new: could not disable depth_ctrl");
    //     // }
    //     last_timepoint_timeout_s = ros::Time(0, 0);
    //     dt_timeout_s = 0.0;
    // }
...
bool HeadingCtrl::enable_cb(std_srvs::SetBoolRequest& req, std_srvs::SetBoolResponse& res)
{
    ...
    if(req.data) {
        if(enabled_) {
            ROS_INFO("heading_ctrl_update_new: already on");
            return true;
        }
        // // start depth controller
        // if(!depth_ctrl_en(true)) {
        //     ROS_ERROR("heading_ctrl_update_new: could not enable depth_ctrl");
        //     res.success = false;
        // }
    ...
    // switch off
    if(!enabled_) {
        ROS_INFO("heading_ctrl_update_new: already off");
        return true;
    }
    enable(false);
    // stop motors
...
void HeadingCtrl::enable(bool enable)
{
    enabled_ = enable;
    if(enable) {
        ROS_INFO("heading_ctrl_update_new: on");
    }
    else {
        ROS_INFO("heading_ctrl_update_new: off");
    }
...
int main(int argc, char* argv[])
{
    ...
    ros::NodeHandle n;
    ros::NodeHandle np("~");
    ROS_INFO("heading_ctrl_update_new: started");
